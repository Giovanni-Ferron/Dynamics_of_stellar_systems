...
def GetData(filename, N, t_conv):
    ...
    return time, pos_ext, vel_ext, pos_CM, vel_CM, CM_p, CM_v
...
time, pos_ext, vel_ext, pos_CM, vel_CM, CM_p, CM_v = GetData(sim_file, N, t_iu_yr)
...
def update_collapse(frame):
    ...
    ax.text(0.875, 0.04, 0., s="t =" + f"{time[frame] / t_iu_yr: .3f}" + " yr", bbox=bbox, color="black", size=10, transform=ax.transAxes)
...
def update_ph(frame):
    ...
    ax_ph.text(0.7, 0.03, s="t =" + f"{time[frame] / t_iu_yr: .3f}" + " yr", bbox=bbox_ph, color="black", size=10, transform=ax_ph.transAxes)
