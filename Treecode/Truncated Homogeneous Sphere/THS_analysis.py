...
K_tot, U_tot = GetLogData("../Truncated Homogeneous Sphere/Logs/" + path + "_log")
...
def update_rho(frame):
    ...
    ax_rho.axvline(a, alpha=0.6, color="grey", linestyle="--", label="$r = b$")
...
def update_V(frame):
    ...
    ax_V.plot(r_grid, V_TH(r_grid), color="crimson", label="Analytical potential profile")
