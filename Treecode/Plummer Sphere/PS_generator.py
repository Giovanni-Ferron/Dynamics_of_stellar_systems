...
density_0 = M * M_sun / (4/3 * np.pi * (np.max(r) * AU)**3)
...
rho_th = lambda rad: 3 * M / (4 * np.pi* b**3) * (1 + rad**2/b**2)**(-5/2) * M_sun * AU**-3
...
K_i = 0.5 * m_i * v**2 * M_sun * v_iu_cgs**-2
U_i = np.zeros(N)
...
for i in tqdm(range(N)):   
    ...
    U_i[idx] += -G_p * (m_i * M_sun)**2 * AU**-1 * np.sqrt(dpos[idx, 0]**2 +
                                                      dpos[idx, 1]**2 +
                                                      dpos[idx, 2]**2)**-1
    for j in range(3):
        dpos[idx, j] += dpos[i, j]
