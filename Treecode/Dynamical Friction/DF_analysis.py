...
display_animation = 1 #True to display the distribution 3D animation
compute_DF = False #True to compute and plot the distribution function

#Relative path of the generated file
path = "DF_N" + str(N) + "_M" + str(M) + "_b" + str(b) + "_Mp" + str(M_p)
...
def GetData(filename, N):
    #Add the perturber to the number N
    N += 1
    ...
    m, M_p = np.sort(np.unique(np.array(data[3:N+3])[:, 0]))
    ...
    CM_p = (np.sum(m * pos_ext[:, 1:, :], axis=1) + M_p * pos_ext[:, 0, :]) / (N * m + M_p)
    CM_v = (np.sum(m * vel_ext[:, 1:, :], axis=1) + M_p * vel_ext[:, 0, :]) / (N * m + M_p)
    ...
    #Perturber position and velocity
    if perturber:
        pos_ext_per = pos_ext[:, 0, :]
        pos_CM_per = pos_CM[:, 0, :]
        vel_ext_per = vel_ext[:, 0, :]
        vel_CM_per = vel_CM[:, 0, :]
    ...
    return time, m, pos_ext, vel_ext, pos_CM, vel_CM, pos_ext_per, vel_ext_per, pos_CM_per, vel_CM_per, CM_p, CM_v
...
time, m_i, pos_ext, vel_ext, pos_CM, vel_CM, pos_ext_p, vel_ext_p, pos_CM_p, vel_CM_p, CM_p, CM_v = GetData(sim_file, N)
...
#Positions and velocities of the perturber in the external frame
x_ext_p, y_ext_p, z_ext_p = pos_ext_p[:, 0], pos_ext_p[:, 1], pos_ext_p[:, 2]
vx_ext_p, vy_ext_p, vz_ext_p = vel_ext_p[:, 0], vel_ext_p[:, 1], vel_ext_p[:, 2]

R_ext_p = np.sqrt(np.sum(pos_ext_p**2, axis=1))
V_ext_p = np.sqrt(np.sum(vel_ext_p**2, axis=1))
            
#Positions and velocities of the perturber in the CM frame
x_CM_p, y_CM_p, z_CM_p = pos_CM_p[:, 0], pos_CM_p[:, 1], pos_CM_p[:, 2]
vx_CM_p, vy_CM_p, vz_CM_p = vel_CM_p[:, 0], vel_CM_p[:, 1], vel_CM_p[:, 2]

R_CM_p = np.sqrt(np.sum(pos_CM_p**2, axis=1))
V_CM_p = np.sqrt(np.sum(vel_CM_p**2, axis=1))
...
K_tot, U_tot, E_tot = GetLogData("../Dynamical Friction/Logs/" + path + "_log")
K_tot *= M_phy * v_iu_cgs**-2
U_tot *= G_p * M_phy**2 * r_phy**-1

CM_K = 1/2 * M * M_phy * CM_V**2 * v_iu_cgs**-2
K_CM = K_tot - CM_K
...
p_tot_ext = m_i * M_phy * np.sqrt(np.sum(vx_ext, axis=1)**2 + 
                              np.sum(vy_ext, axis=1)**2 + 
                              np.sum(vz_ext, axis=1)**2) / v_iu_cgs

L_tot_ext = m_i * M_phy * r_phy * v_iu_cgs**-1 * np.sqrt(np.sum(y_ext * vz_ext - z_ext * vy_ext, axis=1)**2 + 
                                  np.sum(z_ext * vx_ext - x_ext * vz_ext, axis=1)**2 + 
                                  np.sum(x_ext * vy_ext - y_ext * vx_ext, axis=1)**2)

p_tot_CM = m_i * M_phy * np.sqrt(np.sum(vx_CM, axis=1)**2 + 
                              np.sum(vy_CM, axis=1)**2 + 
                              np.sum(vz_CM, axis=1)**2) / v_iu_cgs

L_tot_CM = m_i * M_phy * r_phy * v_iu_cgs**-1 * np.sqrt(np.sum(y_CM * vz_CM - z_CM * vy_CM, axis=1)**2 + 
...
Lambda = np.median(R_CM[0, :]) * np.median(np.sqrt(vx_CM[0, :]**2 + vx_CM[0, :]**2 + vz_CM[0, :]**2)) / m_i**2
n_rel = N / (8 * np.log(Lambda))
t_cross = np.median(R_CM[0, :]) / np.median(np.sqrt(vx_CM[0, :]**2 + vy_CM[0, :]**2 + vz_CM[0, :]**2))
# t_rel = t_cross * n_rel
t_rel = N / (8 * np.log(N)) * np.sqrt(np.median(R_CM[0, :])**3 / M)

#Compute dynamical friction time

v_circ_p = np.sqrt(M_p) * (b**2 + R_CM_p[0]**2)**(-3/4) * R_CM_p[0]
Lambda_p = R_CM_p[0] * V_CM_p[0]**2 / (M_p + m_i)
t_friction = 1.17 * R_CM_p[0]**2 * v_circ_p / (np.log(Lambda) * M)
...
if display_animation:
    ...
    sparse = np.random.choice(np.arange(0, x_CM.shape[0]), 1000)
    zoom_x = 0.8
    zoom_y = 0.8
    zoom_z = 0.2
    ...
    def update_anim(frame):
        ...
        ax.scatter(x_CM[frame, sparse], y_CM[frame, sparse], z_CM[frame, sparse], alpha=0.05, color="darkcyan")
        ax.scatter(x_CM_p[frame], y_CM_p[frame], z_CM_p[frame], s=20, color="crimson")
        ax.plot(x_CM_p[:frame], y_CM_p[:frame], z_CM_p[:frame], color="crimson")
        # ax.plot(x_CM_p[:frame], y_CM_p[:frame], z_CM_p[0], color="green")
        ax.set_xticks(ticks)
        ax.set_yticks(ticks)
        ax.set_zticks(ticks)
        ...
        ax.set_xlim(-zoom_x*np.max(0.5*x_CM[:, sparse]), zoom_x*np.max(x_CM[:, sparse]))
        ax.set_ylim(-zoom_y*np.max(y_CM[:, sparse]), zoom_y*np.max(y_CM[:, sparse]))
        ax.set_zlim(-zoom_z*np.max(z_CM[:, sparse]), zoom_z*np.max(z_CM[:, sparse]))
    ...
    # animation.save("Sim_" + path + ".gif", progress_callback = lambda i, n: print(f'Saving frame {i}/{n}'))
    
#%%----Plot the perturber radius----

x_grid = np.linspace(-b, b, 100)

fig_p, ax_p = plt.subplots(1, 2, figsize=(12, 7))

ax_p[0].plot(x_CM_p, y_CM_p)
ax_p[0].plot(x_grid, np.sqrt(b**2 - x_grid**2), linestyle="--", color="gray")
ax_p[0].plot(x_grid, -np.sqrt(b**2 - x_grid**2), linestyle="--", color="gray")
ax_p[0].set_xlabel("$x\ [AU]$")
ax_p[0].set_ylabel("$y\ [AU]$")

ax_p[1].plot(time_yr, R_CM_p)
ax_p[1].axhline(b, linestyle="--", color="gray")
ax_p[1].set_xlabel("$t\ [yr]$")
ax_p[1].set_ylabel("$R\ [AU]$")
ax_p[1].xaxis.set_major_formatter(ticker.FormatStrFormatter("%.0e"))

fig_p.suptitle("Perturber position over time", y=0.91)
...
edge = 10 * b
r_grid, volume = DivideDistribution(edge, 800, False)
...
for i in range(0, len(time)):
    ...
    #Count only the particles within the edge, and add the number to the p_num
...
r_range = np.linspace(np.min(R_CM), edge, len(r_grid))
...
def update_rho(frame):
    ...
    ax_rho.set_xlim(0, 3 * b)
    ax_rho.set_ylim(0, 1.2 * 3 * M / (4 * np.pi* b**3) * M_phy * r_phy**-3)
    ax_rho.legend(loc="upper right")
    ax_rho.text(0.81, 0.53, s="t =" + f"{time_yr[frame]: .3e}" + " yr", bbox=bbox_rho, color="black", size=10, transform=ax_rho.transAxes)
...
for t in range(len(time)):
    for r in range(len(r_grid)):
        V[t, r] = -G_p * m_i * M_phy * r_phy**-1 * np.sum(np.sqrt((x_CM[t, :] - r_grid[r])**2 + 
                                                    y_CM[t, :]**2 + z_CM[t, :]**2)**-1)
...
def update_V(frame):
    ...
    ax_V.text(0.8, 0.23, s="t =" + f"{time_yr[frame]: .3e}" + " yr", bbox=bbox_V, color="black", size=10, transform=ax_V.transAxes)
...
if compute_DF:
    ...
    K_i = 0.5 * m_i * V_CM**2 * M_phy * v_iu_cgs**-2
    ...
    for t in tqdm(range(len(time))):
        for i in tqdm(range(N)):   
            ...
            U_i[t, :i] += -G_p * (m_i * M_phy)**2 * r_phy**-1 * np.sqrt(dpos[t, :i, 0]**2 +
                                                              dpos[t, :i, 1]**2 +
                                                              dpos[t, :i, 2]**2)**-1
            U_i[t, i+1:] += -G_p * (m_i * M_phy)**2 * r_phy**-1 * np.sqrt(dpos[t, i+1:, 0]**2 +
                                                              dpos[t, i+1:, 1]**2 +
                                                              dpos[t, i+1:, 2]**2)**-1
